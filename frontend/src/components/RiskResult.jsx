  ...
  return (
    <div className="result-card">
      <div className="result-header">
        <div>
          <div className="small-label">SECURITY ANALYSIS COMPLETE</div>
          <h2>Risk Assessment</h2>
        </div>
        <div className="completed-badge">✓ Analyzed</div>
      </div>
      ...
      <div className="risk-main">
        ...
        <div className="risk-info">
          <span className="score-title">SCAM RISK SCORE</span>
      ...
      {/* URL INTELLIGENCE */}
      {hasUrls && (
        <div className="url-intelligence">
          <div className="section-title-row">
            <div>
              <span className="small-label">URL INTELLIGENCE</span>
              <h3>What did we find in the link?</h3>
            </div>
            <span className="url-risk-badge">
              URL Risk: {urlAnalysis.risk}/60
            </span>
          </div>

          <div className="analyzed-url-count">
            🔗 {urlAnalysis.count} URL{urlAnalysis.count === 1 ? "" : "s"} analyzed
          </div>

          {urlAnalysis.indicators?.length > 0 ? (
            <div className="url-indicators">
              {urlAnalysis.indicators.map((indicator, index) => (
                <div className="url-indicator" key={index}>
                  <div className="url-indicator-icon">⚠️</div>
                  <div>
            ...
            <div className="url-safe-message">
              ✓ No suspicious URL characteristics were detected.
            </div>
          )}

          {urlAnalysis.urls?.length > 0 && (
            <div className="url-list">
              <span className="url-list-label">ANALYZED LINKS</span>
              {urlAnalysis.urls.map((url, index) => (
                <div className="url-item" key={index} title={url}>
                  🔗 {url}
                </div>
              ))}
            </div>
      ...
      <div className="signals-section">
        <div className="section-title-row">
          <div>
            <span className="small-label">DETECTION DETAILS</span>
